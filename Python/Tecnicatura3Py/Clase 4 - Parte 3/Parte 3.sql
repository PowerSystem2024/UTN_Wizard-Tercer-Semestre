-- Consulta informacion seleccionada
-- SELECT * FROM persona WHERE id_persona in (0,1)
-- Ingreamos un nuevo elemento a la tabla
-- INSERT INTO persona(nombre, apellido, email) VALUES ('Susana', 'Lara', 'slara@gmail.com')
-- Hacemos la consulta para ver toda la informacion de la tabla
-- SELECT * FROM persona
-- UPDATE persona SET nombre = 'Ivonne', apellido = 'Esperza', email = 'iesparza@gmail.com' WHERE id_persona=3
 SELECT * FROM persona
-- DELETE FROM persona WHERE id_persona=3